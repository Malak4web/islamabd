<template>
    <div class="p-8 font-['Outfit'] bg-[#0a0a0a] min-h-screen text-white">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-3xl font-bold bg-gradient-to-r from-[#d4af37] to-[#f3e5ab] bg-clip-text text-transparent">Code Injections</h1>
        ...
        <div class="bg-[#141414] border border-[#222] rounded-3xl overflow-hidden shadow-2xl">
            <div class="grid grid-cols-12 gap-4 p-6 border-b border-[#222] text-[#555] text-xs font-bold uppercase tracking-widest bg-[#1a1a1a]">
                <div class="col-span-3">Name</div>
                <div class="col-span-2">Location</div>
                <div class="col-span-3">Pages</div>
            ...
            <div v-else class="divide-y divide-[#222]">
                <div v-for="inj in store.injections" :key="inj.id" class="grid grid-cols-12 gap-4 p-6 items-center group hover:bg-[#1a1a1a] transition-colors">
                    ...
                    <div class="col-span-3 text-[#888] text-sm">
                        <span v-if="!inj.pages || inj.pages.length === 0" class="text-white/60">All Pages</span>
                    ...
                    <div class="col-span-2 text-right flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button @click="openModal(inj)" class="p-2 text-[#888] hover:text-[#d4af37] transition-colors" title="Edit">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                        </button>
                        <button @click="deleteInjection(inj)" class="p-2 text-[#888] hover:text-red-500 transition-colors" title="Delete">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" @click.self="closeModal">
            <div class="bg-[#141414] border border-[#222] w-full max-w-4xl rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
                <div class="p-8 border-b border-[#222] flex justify-between items-center shrink-0">
                    <h2 class="text-2xl font-black tracking-tight">{{ editingId ? 'Edit Snippet' : 'New Snippet' }}</h2>
                ...
                <div class="p-8 overflow-y-auto grow space-y-8">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-widest text-[#555] mb-2">Name</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-widest text-[#555] mb-2">Location</label>
                    ...
                    <!-- Page Targeting -->
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-widest text-[#555] mb-3">Page Targeting</label>
                    ...
                    <div>
                        <div class="flex justify-between mb-2">
                            <label class="block text-xs font-bold uppercase tracking-widest text-[#555]">Snippet Code</label>
                    ...
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <span class="font-bold text-sm uppercase tracking-widest text-[#888]">Active</span>
                    </label>
                </div>

                <div class="p-6 bg-[#1a1a1a] border-t border-[#222] shrink-0 flex justify-end gap-4 rounded-b-3xl">
                    <button @click="closeModal" class="px-6 py-3 text-white font-bold hover:text-[#888] transition-colors">Cancel</button>
...
const deleteInjection = async (inj) => {
    if (confirm(`Delete snippet "${inj.name}"?`)) {
