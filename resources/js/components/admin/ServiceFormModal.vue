    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-md">
      <div class="w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in duration-300">
        ...
        <div class="p-10 border-b border-slate-800 flex justify-between items-center">
          <div class="space-y-1">
            ...
            <p class="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">
              {{ isEditing ? 'Update existing service details' : 'Create a new design expertise' }}
            </p>
          </div>
        ...
        <div class="flex px-10 border-b border-slate-800 bg-slate-950/30">
          <button 
            v-for="tab in ['English', 'Arabic', 'Media']" 
            :key="tab"
            @click="activeTab = tab"
        ...
        <div class="flex-1 p-10 overflow-y-auto space-y-10 custom-scrollbar">
          ...
          <div v-if="activeTab === 'English'" class="space-y-8">
            <div class="space-y-4">
              <label class="block text-[10px] font-bold uppercase text-slate-500 tracking-widest">Service Title (EN)</label>
              <input 
            ...
            <div class="space-y-4">
              <label class="block text-[10px] font-bold uppercase text-slate-500 tracking-widest">Detailed Description (EN)</label>
              <textarea 
          ...
          <div v-if="activeTab === 'Arabic'" class="space-y-8" dir="rtl">
            <div class="space-y-4">
              <label class="block text-[10px] font-bold uppercase text-slate-500 tracking-widest text-right">عنوان الخدمة (AR)</label>
              <input 
            ...
            <div class="space-y-4">
              <label class="block text-[10px] font-bold uppercase text-slate-500 tracking-widest text-right">الوصف التفصيلي (AR)</label>
              <textarea 
          ...
          <div v-if="activeTab === 'Media'" class="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div class="space-y-6">
              <label class="block text-[10px] font-bold uppercase text-slate-500 tracking-widest">Iconography</label>
              <div class="relative group border-2 border-dashed border-slate-800 hover:border-amber-500/50 rounded-[2.5rem] p-10 text-center transition-all bg-slate-950/50">
                ...
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ isEditing ? 'Change Icon' : 'Save first to upload' }}</p>
                <p class="mt-2 text-[10px] text-slate-600 uppercase tracking-tighter">SVG or Transparent PNG</p>
              </div>
            </div>

            <div class="space-y-6">
              <label class="block text-[10px] font-bold uppercase text-slate-500 tracking-widest">Cover Narrative</label>
              <div class="relative group border-2 border-dashed border-slate-800 hover:border-amber-500/50 rounded-[2.5rem] p-10 text-center transition-all bg-slate-950/50">
                ...
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ isEditing ? 'Change Image' : 'Save first to upload' }}</p>
                <p class="mt-2 text-[10px] text-slate-600 uppercase tracking-tighter">High Res JPG (max 2MB)</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="p-10 bg-slate-950/50 border-t border-slate-800 flex justify-end gap-6">
          ...
          >
            {{ loading ? 'Saving...' : (isEditing ? 'Update Service' : 'Initialize Service') }}
          </button>
        </div>
      </div>
    </div>
